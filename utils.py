...
shape_predictor = dlib.shape_predictor("{}/models/shape_predictor_68_face_landmarks.dat".format(current_directory))
face_encoder = dlib.face_recognition_model_v1("{}/models/dlib_face_recognition_resnet_model_v1.dat".format(current_directory))
...
def face_encodings(image):
    ...
    return [np.array(face_encoder.compute_face_descriptor(image, face_landmark)) 
            for face_landmark in face_landmarks(image)], face_rects(image)
...
def nb_of_matches(known_encodings, unknown_encoding):
    ...
    small_distances = distances <= 0.5
    # print(distances)
    # print(small_distances)
    # return the number of matches
    return sum(small_distances), sum(small_distances)/len(small_distances)
